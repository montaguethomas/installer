...
resource "azurerm_network_interface" "master" {
  ...
  name                = "${var.cluster_id}-${var.name}${count.index}-nic"
  location            = var.region
...
resource "azurerm_linux_virtual_machine" "master" {
  ...
  os_disk {
    name                 = "${var.cluster_id}-${var.name}-${count.index}_OSDisk" # os disk name needs to match cluster-api convention
    caching              = "ReadOnly"
  ...
  computer_name = "${var.cluster_id}-${var.name}-${count.index}"
